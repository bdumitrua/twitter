...
Route::prefix('users/groups')->middleware(['auth:api'])->controller(UserGroupController::class)->group(function () {
    ...
    // Создать группу
    Route::post('create', 'create')->name('create_user_group');

    Route::middleware(['checkRights:userGroup'])->group(function () {
        // Получить мои группы 
        Route::get('show/{userGroup}', 'show')->name('show_user_group');
        // Изменить данные группы
        Route::patch('update/{userGroup}', 'update')->name('update_user_group');
        // Удалить группу
        Route::delete('destroy/{userGroup}', 'destroy')->name('destroy_user_group');
